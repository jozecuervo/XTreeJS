...
| **Cons** | `blessed`/`neo-blessed` is a low-velocity project with an idiosyncratic, callback-oriented API predating modern async patterns — see the `(screen as any)._treeWidget` and `(screen as any).leave?.()` casts in `src/index.ts` needed to reach into internals blessed's types don't expose. No built-in concept of suspending/resuming cleanly for a child process (`onEditor` in `src/index.ts:521-540` has to manually `leave()`/`enter()` the screen around `$EDITOR`). Layout is imperative and widget-relative rather than a declarative/reactive model (contrast with React-for-terminals frameworks like Ink), so UI updates go through one big `refreshUI()` (`src/index.ts:105-173`) that manually re-reads state into each widget. |
